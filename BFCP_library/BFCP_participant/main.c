	...
	while(fgets(line, 79, stdin) != NULL) {
		...
		switch(*lineptr) {
			...
			case 'r':
				...
				error = scanf ("%u", &floorID);
				BFCP_CHECK_INTEGER;
				node = create_floor_list(floorID, NULL);
				printf("Enter another FloorID: (0 to stop inserting floors)\n");
				error = scanf ("%u", &floorID);
				BFCP_CHECK_INTEGER;
				while (floorID != 0) {
					node = insert_floor_list(node, floorID, NULL);
					printf("Enter another FloorID: (0 to stop inserting floors)\n");
					error = scanf ("%u", &floorID);
					BFCP_CHECK_INTEGER;
				}
			...
			case 'a':
...
#endif
				list_floor = create_floor_list(floorID, text);
				free(text);
				...
				while(floorID != 0) {
					list_floor = insert_floor_list(list_floor, floorID, text);
					free(text);
					printf("Enter another FloorID: (0 to stop inserting floors)\n");
					error = scanf ("%u", &floorID);
					BFCP_CHECK_INTEGER;
