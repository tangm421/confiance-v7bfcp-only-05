...
#ifndef WIN32
	if((inet_aton(IP_address_server,&server_addr.sin_addr)) <= 0) {
#else
...
int bfcp_insert_floor_participant(conference_participant participant, unsigned short int floorID)
{
	...
	if(participant == NULL)
   	...
   	participant->pfloors = insert_floor_list(participant->pfloors, floorID, NULL);
...
bfcp_floors_participant *create_floor_list(unsigned short int floorID, char *status_info)
{
...
bfcp_floors_participant *insert_floor_list(floors_participant floor_list, unsigned short int floorID, char *status_info)
{
	if(floorID <= 0)
		return NULL;
...
int remove_floor_list(floors_participant floor_list)
{
