...
#define BFCP_SEND_CHECK_ERRORS(fd)				\
	if(error < 0) {						\
		if(arguments != NULL)				\
...
struct bfcp_server *bfcp_initialize_bfcp_server(unsigned short int Max_conf, unsigned short int port_server, int(*notify_to_server_app)(bfcp_arguments *arguments, int outgoing_msg), int transport, char *certificate, char *privatekey)
{
#ifdef WIN32
	WSADATA wsaData;
